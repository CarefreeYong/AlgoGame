...
import { Direction, RotationDirection, PathDirection, GridType } from '@/types'
...
const search = (resumed: boolean, finalCallback: (vectors: Vector[]) => void): void => {
    ...
    if (!initialDirectionChanged) {
        ...
        !isPassable(initialSearchProbeAdjacentGrids[initialDirection] as Grid) && (
    ...
    searchIntervalId = setInterval(async () => {
        ...
        // 更新搜索指针
        searchProbe.value = stack.value[stack.value.length - 1] as Vector
        ...
        const endDirection = searchProbeAdjacentGrids.findIndex(({ position }) => String(position) === String(route.value.end)) as Direction | -1
        switch (endDirection) {
            case -1: {
                break
            }
        ...
        if (initialDirectionChanged) { // 2. 靠墙后，按照道路交通中的右侧行驶规则进行搜索直到找到终点
            ...
            let popped = false
            while (true) {
                const lastSearchProbe = stack.value[stack.value.length - 1] as Vector
                ...
                for (const dir of checkedSequence) {
                    if (isPassable(lastSearchProbeAdjacentGrids[dir] as Grid)) { lastSearchProbe.direction = dir; break }
                }
            ...
            !isPassable(nextSearchProbeAdjacentGrids[searchProbe.value.direction] as Grid) && (
